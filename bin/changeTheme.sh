...
GHOSTTY_LOCAL_THEME_FILE="$GHOSTTY_DIR/theme.local.conf"
EZA_DIR="$CONFIG_HOME/eza"
EZA_LOCAL_THEME_FILE="$EZA_DIR/theme.local.sh"
FZF_DIR="$CONFIG_HOME/fzf"
FZF_LOCAL_THEME_FILE="$FZF_DIR/theme.local.sh"
LAZYGIT_DIR="$CONFIG_HOME/lazygit"
LAZYGIT_LOCAL_THEME_FILE="$LAZYGIT_DIR/theme.local.yml"
...
write_ghostty_theme_file() {
    local flavour="$1"
    local theme_file="$GHOSTTY_DIR/themes/catppuccin-${flavour}.conf"
    ...
    mkdir -p "$GHOSTTY_DIR"
    cat >"$GHOSTTY_LOCAL_THEME_FILE" <<EOF
theme = catppuccin-${flavour}.conf
EOF
}
...
write_fzf_theme_file() {
    local flavour="$1"
    mkdir -p "$FZF_DIR"
    ln -sfn "themes/catppuccin-${flavour}.sh" "$FZF_LOCAL_THEME_FILE"
}

write_lazygit_theme_file() {
    local flavour="$1"
    mkdir -p "$LAZYGIT_DIR"
    ln -sfn "themes/catppuccin-${flavour}.yml" "$LAZYGIT_LOCAL_THEME_FILE"
}
...
reload_ghostty() {
    if systemctl --user status app-com.mitchellh.ghostty.service >/dev/null 2>&1; then
...
set_theme() {
    ...
    write_ghostty_theme_file "$flavour"
    write_nvim_theme_file "$flavour"
    write_eza_theme_file "$flavour"
    write_fzf_theme_file "$flavour"
    write_lazygit_theme_file "$flavour"
...
init_theme() {
    ...
    if [ ! -f "$GHOSTTY_LOCAL_THEME_FILE" ]; then
        write_ghostty_theme_file "$(theme_to_flavour "$theme")"
    fi
    ...
    if [ ! -e "$FZF_LOCAL_THEME_FILE" ]; then
        write_fzf_theme_file "$(theme_to_flavour "$theme")"
    fi

    if [ ! -e "$LAZYGIT_LOCAL_THEME_FILE" ]; then
        write_lazygit_theme_file "$(theme_to_flavour "$theme")"
    fi
