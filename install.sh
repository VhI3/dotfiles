...
Next step for a fresh machine:
  setup-local-configs

Then edit the generated local files and run:
...
Options:
  --all             Run the full stack and link dotfiles
  --fresh-minimal   Alias for --all; intended for a fresh minimal Debian install
...
echo "  3) wayland  — sway, waybar, ghostty, kitty, rofi, swaync, grim, swaylock, kanshi"
...
echo "  f) fresh    — same as all, optimized for a fresh minimal Debian install"
echo "  l) link     — symlink configs only"
echo "  q) quit"
echo ""

while true; do
    read -rp "Select layer(s) [0-9/a/f/l/q]: " choice
    case "$choice" in
